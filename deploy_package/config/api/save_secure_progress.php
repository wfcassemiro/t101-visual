...
// VALIDAÇÕES DE SEGURANÇA ANTI-FRAUDE (MUITO RELAXADAS PARA TESTE)
$security_violations = [];

// MODO TESTE: Validações muito mais permissivas
writeSecurityLog("MODO TESTE: Validações de segurança relaxadas");

// 1. Verificar se o progresso não excede MUITO a duração (tolerância grande)
if ($total_duration_seconds > 0 && $watched_seconds > ($total_duration_seconds + 3600)) { // +1 hora de tolerância
    $security_violations[] = "Tempo assistido excede muito a duração total";
    writeSecurityLog("VIOLAÇÃO TESTE: Tempo assistido ($watched_seconds) > duração + tolerância (" . ($total_duration_seconds + 3600) . ")");
}

// 2. Verificar atividade suspeita (muito tolerante)
if ($suspicious_activity_count >= 20) { // 20 ao invés de 3
    $security_violations[] = "Atividades suspeitas excessivas";
    writeSecurityLog("VIOLAÇÃO TESTE: Atividades suspeitas: $suspicious_activity_count");
}

// 3. Verificar se fraude foi detectada pelo cliente
if ($fraud_detected) {
    $security_violations[] = "Fraude detectada pelo sistema cliente";
    writeSecurityLog("VIOLAÇÃO TESTE: Fraude detectada pelo cliente");
}
...
try {
    ...
    if ($existing_log) {
        ...
        if ($watched_seconds > $current_progress) {
            // MODO TESTE: Muito mais tolerante com aumentos grandes
            $max_realistic_increase = $current_progress + 3600; // + 1 hora de tolerância para teste
            
            if ($watched_seconds > $max_realistic_increase) {
                writeSecurityLog("ALERTA TESTE: Aumento muito grande mas permitindo - Atual: $current_progress, Novo: $watched_seconds, Diferença: " . ($watched_seconds - $current_progress));
                // No modo teste, vamos permitir mesmo assim
            }
            ...
            if ($rowCount > 0) {
                ...
                $response['message'] = 'Progresso de teste atualizado.';
                $response['security_info'] = [
                ...
                writeSecurityLog("SUCESSO TESTE: Progresso atualizado: $current_progress -> $watched_seconds");
            } else {
                $response['message'] = 'Erro ao atualizar progresso de teste.';
            ...
            writeSecurityLog("INFO TESTE: Progresso não atualizado: enviado $watched_seconds <= atual $current_progress");
        }
    } else {
        // Criar novo log
